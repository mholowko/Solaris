...
\usepackage[mincitenames=2, maxcitenames=2,style=numeric, sorting = none]{biblatex}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
...
do not yet provide uncertainty levels to guide exploration \hl{with the notable exception of ART} \cite{Radivojevic2020} .
Furthermore large data sets may not be available for a genetic part of interest, for instance the RBS.
Hence there is a need to develop methods for training predictors on smaller data sets and generate predictions for both their mean values and uncertainties \hl{of the predictions} in the Learn phase of DBTL. The predictions of characteristics of interest for
...
Figure \ref{fig: Scatterplot} shows how our implementation of the Gaussian Process algorithm performed in terms of predictions (on an hold out test set) in each round. As expected, the predictions in Round 0 were poor due to the use of approximate data.
...
In each round, we select some data points for exploration of the areas in which we have few or no tested data points.
...
In other words, given an RBS with high TIR, multiple sequence edits need to occur to reach another RBS with high TIR \hl{(we assume here our data set includes most of the highest RBSs for each distance - sequence combination)}.
...
\begin{itemize}
    \item A detailed text description of machine learning methods, including Gaussian Process regression, choices of kernels, batch recommendation, experimental settings, additional results and model evaluation on RBS calculator data.
    \item Figure S1-S12.
    \item Table S1-S2.
\end{itemize}
