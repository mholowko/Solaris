...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \caption{}
        \includegraphics[scale=0.3]{plots/Main_Paper/tsneplot.pdf}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \caption{}
    ...
    \textbf{E)} Exploitation v.s. Exploration for Bandit 1-3. Blue-hued points represent exploitation, those hued red represent exploration.}
...
In \ref{fig: Swarmplot and Quantplot}D we show a tSNE plot depicting the experimental space.
...
For example, the circled clusters 
% positioned in the top centre
labelled as "G-Rich Clusters" have been actively recommended by the algorithm.
...
Finally, in Figure \ref{fig: Swarmplot and Quantplot} E) we coloured the swarmplot for Bandit 1-3 groups as the scores representation the exploration-exploitation level. The scores are calculated based on the projection of prediction standard deviation and mean pairs (for details see Supplementary). 
We can see the RBSs with high TIRs (blue, with high predicted mean) tend to exploit the design space; the explorative points (red, with high predicted standard deviation) gives relatively low TIR but expands the knowledge of about the unknown design space. 
Another observation is that a higher ratio of the points in Bandit-3 compared the other two groups are exploitative. 
This is an expected behaviour of our algorithm since we have set a large exploitation rate for the last group (Bandit-3).
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.49\textwidth}
         \centering
         \caption{}
         \includegraphics[scale=0.4]{plots/Main_Paper/SD.pdf}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.49\textwidth}
         \centering
         \caption{}
         \includegraphics[scale=0.6]{paper/plots/Main_Paper/SDvsTIR.pdf}
     \end{subfigure}
