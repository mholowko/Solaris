...
The regression task in the DBTL cycle is more challenging than the large-scale data based regression tasks \cite{}.
...
\mengyan{Do we even want to put R2 score in?}\\
...
We reported the Spearman and $R^2$ score in each plot's title on test data only. 
...
\mengyan{We also show $R_2$ score for the reader's interest, which quantifies value-based prediction evaluation. Similarly, the $R_2$ increases along with rounds increase. }
