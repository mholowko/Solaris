...
\cheng{I suggest reporting everything that is 0.95 and above as "high" (assuming the ratio normalized TIR scale)}

The \textbf{Story} (3 sentences):
\begin{enumerate}
    \item we automate the design of sequences for finding high TIR RBS
    \begin{enumerate}
        \item DBTL
    \end{enumerate}
    \item the ML method discovers biological knowledge (validates existing knowledge), including the highest TIR ever
    \begin{enumerate}
        \item high G
        \item diverse library
        \item edit distance $>$ 2
    \end{enumerate}
    \item BioFoundry helps ML (good data), and ML helps BioFoundry (more automation)
    \begin{enumerate}
        \item Sequences have small technical variation
        \item UCB much more efficient than random
    \end{enumerate}
\end{enumerate}
...
% with the machine learning process emphasised.
...
% We use the Upper Confidence Bound multi-armed Bandit algorithm to recommend sequences to be Built and Tested next.
...
We measure the TIR of reference RBS as experimental control. 
We then obtain the normalised TIR (called \textit{TIR ratio}) by taking the ratio between the raw TIR and the average TIR of reference sequences in each round.
