...
\begin{figure}[h]
    ...
    \includegraphics[scale=0.35]{plots/Main_Paper/histogram.pdf}
    \includegraphics[scale=0.35]{plots/Main_Paper/tsneplot.pdf}
...
One of the challenging for applying machine learning techniques on synthetic biology is lacking of high-quality labelled data. 
The available RBS library \cite{jervis2018machine} only cover 56 unique labelled RBS 
...
Figure \ref{fig: SD} shows the distribution of relative error for each tested RBS, calculated as the average TIR from 6 biological replicates divided by the given samples standard deviation.
...
\begin{figure}[h]
    ...
    \caption{\textbf{Distribution of standard deviation of tested samples} Join tplot showing the distribution of relative error calculated as the average TIR from 6 replicates divided by the given samples standard deviation. TIR is given in terms of raw number.}
    \label{fig: SD}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.32]{plots/Main_Paper/scatter_abc1_FF.pdf}
...
\subsection{Important characteristics of the evaluated sequences}

A genetic part library should display a number of characteristics to be deemed useful.
First quality expected from a good library is a wide range of relevant trait values (in our case it is the TIR) so that designed genetic devices can be appropriately optimised.
Secondly, it is very beneficial for the library to show a number of parts with similar numerical values for the relevant trait, but with different sequences, so that a designer would not have to settle on suboptimal part due to, for example, assembly incompatibility.
Finally, a good library has to be reliable - if the numbers provided in the library are unreliable designers will be reluctant to use the data set.
Figure \ref{fig:Library characteristics} shows how our library conforms to these requirements.
...
Figure \ref{fig:Library characteristics} shows what edit distance is required for positive change in TIR for RBS with high and medium TIR.
