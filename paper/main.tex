...
learning method) only solves half of the problem. The designer still needs to choose from the large number of 
...
Hence the Design phase of TIR has two steps: first to predict the TIR value for each potential RBS sequence,
and second to select a small set of RBS sequences based on their predicted TIR values.\\

To achieve the first step, one could use the existing RBS predictors. \textcite{Reeve2014} surveys three main RBS calculators, all predicting the TIR based on the thermodynamic properties of the RBS and the ribosome \cite{Seo2013,Na2010,Salis2009}. 
...
Recent work has explored the use of machine learning algorithms to generate new models and predictors for use in synthetic biology, vastly improving the DBTL cycle's performance \cite{Camacho2018,Radivojevic2020,LAWSON2021}.
...
However large data sets may not be available for a genetic part of interest, for instance the RBS. Hence there is a need to develop methods for training predictors on smaller data sets in the Learn phase of DBTL. The trained predictors
can then be used in the first step of the Design phase to predict characteristics of interest for a potential variant.\\
...
In particular, we develop a method for the Learn phase, and use the learned predictor for the first step of the Design phase.
Furthermore we develop a method for the second step of the Design phase,
