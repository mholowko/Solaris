...
To address this problem, we have created a machine learning guided Design-Build-Test-Learn cycle for the experimental design of bacterial RBSs to show how small genetic parts can be reliably designed \hl{using relatively small, high-quality data sets}.
We used Gaussian Process Regression for the Learn part of cycle and the Upper Confidence Bound multi-armed Bandit algorithm for the Design of genetic variants to be tested in vivo.
...
\hl{Part design, understood as modifying the sequence of a part for it to meet specific requirements, is a central problem of synthetic biology. This is} because, it is the genetic parts that are ultimately combined into more complicated genetic circuits that produce desired functions in the target organisms. 
...
\hl{For instance}, according to  \textcite{Reeve2014} there are three main RBS calculators, all predicting the TIR based on the thermodynamic properties of the RBS and the ribosome \cite{Seo2013,Na2010,Salis2009}. 
...
\hl{However, such big data sets are still generated relatively rarely and there is a need to develop methods operating on smaller data sets.}\\
...
On the other hand, the designer could use machine learning methods to make the predictions, but these usually require large data sets.
To address this problem, we have created a machine learning guided Design-Build-Test-Learn cycle for the experimental design of bacterial RBSs to show how small genetic parts can be reliably designed using even relatively small, but high-quality data sets.}
...
We demonstrate how machine learning (ML) algorithms can be incorporated into the DBTL cycle to predict (Learn) and recommend (Design) variants of a bacterial RBS with the goal of optimising associated protein expression level \hl{in the specific genetic context (i.e. with the specific bases upstream and downstream of the investigated RBS sequences)}. 
...
\hl{GPR, as a non-parametric Bayesian regression algorithm, assumes that the reward function is sampled from a Gaussian Process and has been shown to  perform well with a small amount of data in biological prediction tasks} \cite{Rasmussen2004, desautels2014parallelizing}. 
...
The two types of algorithms cooperate with each other and provides powerful tools for DBTL cycle in genetic parts design \cite{srinivas2012information, desautels2014parallelizing}.
\hl{This way, we were able to rationally increase production of our target protein compared to the very strong benchmark RBS by up to 35\%}. \\
...
(as shown by UNI and PPM), is that most of the exploration will result in low TIR.
...
\hl{To provide the predicted TIR with the confidence interval needed for the UCB algorithm in the Design part, we have selected a regression algorithm, Gaussian Process Regression (GPR),  which can predict not only the means, but also uncertainty estimations for our data points. }
...
However, since the predictions are only used for recommendations in the Design part, instead of precise predictions of the mean for each RBS, we only need to provide a valid ranking for both the predicted mean TIR and uncertainties, and thus the ranking of the UCB scores.
...
Because of that, we provide the evaluation of the Learn part in terms of a ranking-based Spearman correlation coefficient, which has been shown to be a more suitable evaluation metric when the prediction is used for recommendation tasks than coefficient of determination ($R^2$) or Pearman correlation coefficient}
\cite{Schober2018,  Kang2019}.\\

Additionally, the 
% obtained values of the coefficient of determination and Spearman correlation coefficient 
performance of Learn part 
is also influenced by the exploration-exploitation balance in the Design part.
...
\begin{figure}[!ht]
    ...
    The Spearman correlation coefficient (with corresponding p-value; calculated using test data only) are provided for each plot.
