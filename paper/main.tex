...
\maciej {We don't have to be that specific in the abstract, remember that we only have 150 words.}
...
\maciej{In this case translate means making recommendations from previous data.}
...
\maciej{It is, our data set is still considered pretty big :)}
...
whereas the exploration points give relatively low TIRs. Note that the exploration is necessary to expand our knowledge to unknown parts of the design space.}\\
...
To provide the predicted TIR with the confidence interval needed for the UCB algorithm in the Design part, we select a regression algorithm, Gaussian Process regression (GPR),  which can generate not only predictions, but also uncertainty estimations. 
...
At the early iterations, we have a limited number of data points and relatively high variants due to the measurement noise. 
Fortunately, since the predictions are only used for recommendations in the Design part, instead of precise predictions, we only need to provide a valid ranking for both predicted TIR and uncertainties, and thus the ranking of UCB scores. 
So we provide the evaluation of Learn part in terms of a ranking-based Spearman correlation coefficient, which has been shown as a more suitable evaluation metric when the prediction is used for recommendation tasks than $R^2$ or Pearman correlation coefficient
...
In (A)(B)(C), we show at round $t = 1,2,3$ respectively, we train our predictor based on the previously obtained data (round 0 to $t-1$) and show the predictions on both the training data (orange) and recommendations suggested by the Design part (i.e. test data, blue). 
We reported the Spearman score in each plot's title on test data only. 
X-axis and y-axis are true TIR from our Test part of the DBTL cycle and the predicted TIR from our predictor.
In (D), we train our predictor on randomly chosen 80\% of all available data, and test on the rest 20\% data. 
As expected, the predictions in Round 0 were poor due to the use of approximate data. 
The predictions improved for the subsequent rounds, 
% from R\textsuperscript{2} (coefficient of determination) of 0.067 for Round 0 to R\textsuperscript{2} of 0.213 for Round 3.
...
In each round, we select some data points for exploration on the areas we have few or none data points.
...
\begin{figure}[!ht]
    ...
    The Spearman correlation coefficient (with corresponding p-value) are provided for each plot.
