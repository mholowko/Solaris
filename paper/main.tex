...
\affil[1]{AIML FSP}
\affil[2]{ANU}
\affil[3]{CSIRO Synthetic Biology Future Science Platform, CSIRO Land and Water, Brisbane, QLD 4102, Australia}
...
Figure \ref{fig: Flowchart} shows the DBTL cycle with the machine learning process emphasised.
...
\begin{enumerate}
    ...
    \item 30 RBS sequences randomised based on the position probability matrix (PPM) generated from all the naturally occurring RBS sequences in \emph{E. coli} genome
    \item 60 RBS sequences recommended by our implementation of recommendation algorithm based on a data set obtained from literature, as described below.
\end{enumerate}{}


To generate algorithm driven designs for the 0th round we have used the data set available in \textcite{jervis2018machine}.
...
Kernel functions implicitly embed DNA sequences into high-dimensional feature space which enables capturing similarities and distances between data points.  
...
For subsequent rounds, we chose to use the weighted degree kernel with shifts. \\
