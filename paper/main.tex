...
First problem to be tackled was how to embed the sequence to give our RBS sequences numerical form. As a Gaussian Process regression has been used, we have investigated use of different types of kernels (also called covariances) for embedding \cite{Ben-Hur2008}. 
...
The recommendations were made by Upper Confidence Bound based Bandit algorithm. 
...
As such, it is very useful in solving problems like the one presented here - a big pool of potential designs, but only limited time and money that can be used for finding the optimal one.\\
