...
We show these results in Supplementary Figures} \ref{fig: Swarmplot and Quantplot Salis}, \hl{but in short, our approach was able to identify all the sequences with TIR higher than the benchmark's within the first 2 simulated rounds of experiments in 81 out of 100 runs and in 3 simulated rounds of experiments in 99 out of 100 runs.
...
Another interesting characteristic uncovered by our research is the perceived editing distance between two sequences required for  improvement in the TIR when the given RBS' TIR is already high \hl{(we assume here our data set includes most of the highest RBSs for each distance - sequence combination)}.
...
\hl{In our work we have steered the recommendation process towards exploitation as we progressed through the rounds, in line with other works addressing this balance} \cite{Radivojevic2020}.
