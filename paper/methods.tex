...
To find RBS sequences with the highest possible TIR score after a total number of rounds $N$,  we consider our experimental design problem as a sequential optimisation of an unknown reward function $f: \mathcal{D} \rightarrow \mathbb{R}$, where $\mathcal{D}$ is the set containing all RBS sequence points in the design space, and $f(\mathbf{x})$ is the TIR score at 20-base RBS sequence $\mathbf{x} \in \{A,C,G,T\}^{20}$.
...
Since GPR assumes similar sequences would have similar labels (e.g. by knowing $x=2$ we can gain information of $x=2.5$ as well), we prefer to not waste time and money on labelling sequences with high similarities in the same batch.
...
A key property of Gaussian Process regression is that the predicted standard deviation depends only on observed points (i.e. features), not on the labels of those observed points. 
...
The strategy here is to recommend sequences sequentially by updating the UCB score with the predicted standard deviation of the previously recommended points.\\
