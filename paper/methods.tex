...
\mengyan{The pipeline for Bandit-0 is reported in Supplementary XX.}
...
For regression model, we have used a Bayesian non-parametric approach called \textit{Gaussian Process Regression (GPR)} \cite{Rasmussen2004}, which provides not only the predicted TIR scores, but also the uncertainty level of the predictions. 
...
\mengyan{maybe we need to introduce spectrum kernel as well since we used it for round 0.}
...
One can make use of this property to design batch upper confidence bound (BUCB) algorithm \cite{desautels2012parallelizing}.
