...
\hl{We have evaluated our model using data generated using the RBS calculator.} \cite{Salis2009} 
\hl{To do this, we have generated TIR predictions for all 4096 RBS sequences using the RBS calculator ver. 2.0. 
Next, we have applied our approach to this data set - first we have chosen 90 sequences at random and after teaching the algorithm using the obtained numbers we have generated 90 further sequences to be tested (using UCB for recommendations) and did this for a total of 5 rounds.
We have repeated this experiment 100 times to increase our confidence in the results, by doing so we have found that the approach can identify all sequences with TIR above the benchmark in 2 rounds in 81 cases and in 3 rounds in 99 cases out of 100.}
Figure \ref{fig: Swarmplot and Quantplot Salis} \hl{shows plot analogues to these in Figure} \ref{fig: Swarmplot and Quantplot} \hl{ but for one of the runs using the RBS calculator data.
The apparent drop in TIR of the recommended sequences after round 1 is showing our processes efficacy - the best sequences are recommended early. }
