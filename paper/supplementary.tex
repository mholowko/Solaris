...
Recall the UCB score of a sequence $\mathbf{x}_i$ at Round $t$ is defined as 
\begin{align*}
% \label{Eq: UCB}
...
We show the predicted TIR (x-axis) v.s. predicted STD (y-axis) in Figure \ref{fig:exploitationvsexplorationdetail} in orange points. 

To compare the predicted TIR and STD in the same dimension (basis), we project the (predicted TIR, predicted STD) pair to the diagonal line (the grey dashline) crossing (0,1) and (1,0).
...
Finally, we colour those projected points using the RdBu spectrum (according to either of the two dimension of the projected points), where the points with high STD (exploration) are shown in red and those with high mean TIR (exploitation) are shown in blue.
These colors are correspond to ones shown in Figure \mbox{\ref{fig: Swarmplot and Quantplot}C}.
...
\begin{figure}[!ht]
   ...
   \caption{\textbf{Exploration-Exploitation scores illustration.} We show the predictions for each sequence from Round 1-3 in orange (with the same settings as Figure \mbox{\ref{fig: Scatterplot}}).
    We explain how the colors are generated in Section \ref{subsec: Exploration-exploitation visualisation}, which are correspond to the colors shown in Figure \mbox{\ref{fig: Swarmplot and Quantplot}C.}
   }
