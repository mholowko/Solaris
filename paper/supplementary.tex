...
\begin{figure}[!ht]
   ...
   Finally, we colour those projected points using the RdBu spectrum where the points with high STD (exploration) are shown in red and those with high mean TIR (exploitation) are shown in blue, and these correspond to the points shown in Figure \mbox{\ref{fig: Swarmplot and Quantplot}C}.
   }
...
\begin{figure}[!ht]
    ...
    Compared to the results shown in Figure \mbox{\ref{fig: Scatterplot}}, here we have applied kernel normalisation described in Section \mbox{\ref{supp: Normalisation of Kernel}}. Although the $R^2$ scores decrease considerably compared with Figure \mbox{\ref{fig: Scatterplot}}, the Spearman correlation remains similar.
    }
